...
export function AppSidebar({ onCloseMobile }: { onCloseMobile?: () => void }) {
  ...
  const mainNav = [
    ...
    { name: 'Explore', href: '#', icon: Compass, comingSoon: true },
  ];
            ...
            {mainNav.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.comingSoon && (
                    <span className="text-[10px] bg-slate-800 text-amber-400/90 font-medium px-2 py-0.5 rounded-full border border-slate-700">
                      Soon
                    </span>
                  )}
                </Link>
